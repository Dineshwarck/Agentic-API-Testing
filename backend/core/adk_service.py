...
Expected Status: {test_case.expected_status}
    ...
    async def generate_from_swagger(self, swagger_url: str):
        ...
        try:
            ...
            parsed_data = {}
            try:
                # Naive cleanup of markdown code blocks
                clean_json = response_text.replace('```json', '').replace('```', '').strip()
                parsed_data = json.loads(clean_json)
            except json.JSONDecodeError:
                print("DEBUG: Failed to parse regular JSON, trying to find substring")
            ...
            return {
                "success": True,
                **stats,
                "count": stats["test_cases_generated"] # Frontend expects 'count'
            }
